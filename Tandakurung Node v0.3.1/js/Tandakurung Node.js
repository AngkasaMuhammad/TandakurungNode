...
`	)
				...
				if(dipejet === 1){
					...
					if(!oini && hov0ini.children.length){
						...
						for(let naA in arr_asli){
							...
							nodebaru.dataawal	= arr_asli[naA].dataawal
							//nodebaru.akhir	= arr_asli[naA].akhir
							//nodebaru.awal	= arr_asli[naA].awal
							nodebaru.dataakhir = arr_asli[naA].dataakhir
				...
				case 'klik':
					...
					for(let naA = 0;naA<arr.length;naA++){
						...
						for(let naB in c){
							...
							//h0
							let isi = {
								dataawal	:vaB.dataawal	,
								//awal	:vaB.awal	,
								nodes	:[]	,
								//akhir	:vaB.akhir	,
								dataakhir	:vaB.dataakhir	,
								tab	:''	,
							}
							h0[naA].push(isi)
					...
					for(let naA = 0;naA<h1.length;naA++){
						...
						for(let naB in vaA){
							let vaB = vaA[naB]//{}
							let texdataawal = vaB.dataawal.length?tambahtab(vaB.tab,vaB.dataawal,):''
							//let texawal = vaB.awal.length?(vaB.tab+vaB.awal):''
							//let texakhir = vaB.akhir.length?(vaB.tab+vaB.akhir):''
							let texdataakhir = vaB.dataakhir.length?tambahtab(vaB.tab,vaB.dataakhir,):''
							h1.splice(naA+naB*3,0,
								texdataawal	,
								//texawal	,
								vaB.nodes	,
								//texakhir	,
							...
							//
							vaB.divdataawal	.textContent = texdataawal
							//vaB.divawal	.textContent = texawal
							//vaB.divakhir	.textContent = texakhir
							vaB.divdataakhir	.textContent = texdataakhir
						}
	...
	let garishubung = (e,o,dt,)=>{
		...
		for(let naA = 0;naA < c.length;naA++){
			...
			cx.beginPath()
			let xawal = ml0?(ml0[4]+mh0[4]*2):0
			let yawal = ml0?(ml0[5]+mh0[5]*2):0
			/*
			;ml0?
				cx.moveTo(
					ml0[4]+mh0[4]*2	,
	...
	let warnaeve = e=>{
		htmltexeve(e)
		cla('dataawal')[0].style.color = 
		cla('dataakhir')[0].style.color = e.currentTarget.value
	}
	
	cla('judul')[0].addEventListener('input',htmltexeve,)
	cla('judul')[0].addEventListener('change',htmltexeve,)
	cla('dataawal')[0].addEventListener('input',htmltexeve,)
	cla('dataawal')[0].addEventListener('change',htmltexeve,)
	cla('dataakhir')[0].addEventListener('input',htmltexeve,)
	cla('dataakhir')[0].addEventListener('change',htmltexeve,)
	...
	let bukafile = e=>{
		...
		for(let naA = 0;naA<arr.length;naA++){
			...
			for(let naB in c){
				...
				//jagajaga kalo ada versi pertama & versi v1
				node.dataawal =
					(typeof vaB.dataawal	!== 'undefined'?(vaB.dataawal	):'')+//v1
					(typeof vaB.data	!== 'undefined'?(vaB.data+'\n'+vaB.awal	):'')
				node.dataakhir =
					(typeof vaB.dataakhir	!== 'undefined'?vaB.dataakhir	:'')+//v1
					(typeof vaB.akhir	!== 'undefined'?vaB.akhir	:'')
	...
	cla('dataawal')[0].addEventListener('keydown',tekanTab,)
	cla('dataakhir')[0].addEventListener('keydown',tekanTab,)
	...
	let pilihnode = o=>{
		...
		cla('warna')[0].value = 
		cla('dataawal')[0].style.color = 
		cla('dataakhir')[0].style.color = o.warna
		cla('dataawal')[0].value = o.dataawal
		cla('dataakhir')[0].value = o.dataakhir
